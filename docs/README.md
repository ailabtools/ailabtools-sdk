...
Official SDK documentation for AILabTools AI image and portrait APIs, including Node.js TypeScript, async Python, Go, Dart, Flutter, PHP, Ruby, Rust, Java, Kotlin/Android, C#/.NET, and Swift examples for background removal, image upscaling, object removal, face retouching, hairstyle changer, cartoon avatar generation, skin analysis, virtual try-on, and more.
...
- Rust file fields use `FileInput::from_path` or `FileInput::from_bytes`.
...
- Rust SDK source and crates.io metadata: [`packages/rust`](../packages/rust)
